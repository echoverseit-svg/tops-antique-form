-- Migration script to update the schema (v4)
...
CREATE TABLE IF NOT EXISTS achievement_categories (
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT unique_category_name_level UNIQUE (name, level)
);
...
CREATE TABLE IF NOT EXISTS application_reviews (
    ...
    notes TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT fk_application_review
        FOREIGN KEY (application_id) 
        REFERENCES tops_applications(id)
        ON DELETE CASCADE,
    CONSTRAINT fk_reviewer_review
...
CREATE TABLE IF NOT EXISTS application_comments (
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT fk_application_comment
        FOREIGN KEY (application_id) 
        REFERENCES tops_applications(id)
        ON DELETE CASCADE,
    CONSTRAINT fk_reviewer_comment
        FOREIGN KEY (reviewer_id) 
        REFERENCES reviewers(id)
);
...
-- 8. Insert sample achievement categories
